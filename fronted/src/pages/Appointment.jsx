...
const Appointment = () => {
  ...
  const {doctors, bookAppointment, cancelAppointment, hasAppointmentConflict, getUserAppointments, isAuthenticated} = useContext(AppContext);
  ...
  const [currentDoctorAppointment, setCurrentDoctorAppointment] = useState(null)
    ...
    if (doctors && doctors.length > 0 && docid) {
      ...
      if (foundDoc) {
        const related = doctors.filter(doc => 
          doc.speciality === foundDoc.speciality && doc._id !== docid
  ...
  useEffect(() => {
    if (isAuthenticated()) {
      ...
      const existingAppointment = appointments.find(apt => 
        apt.doctorId === docid && apt.status === 'confirmed'
      );
    ...
    <div className='max-w-6xl mx-auto p-4 sm:p-6 lg:p-8'>
        ...
        <div className='lg:w-1/3 relative'>
          <div className='aspect-square lg:aspect-[3/4] bg-gradient-to-br from-blue-50 to-indigo-100 p-6 flex items-center justify-center'>
            <img 
              ...
              onLoad={() => console.log('Image loaded successfully:', docInfo?.image)}
            />
          </div>
        </div>
        ...
        <div className='lg:w-2/3 p-6 lg:p-8 space-y-6'>
          ...
          <div className='space-y-4'>
            <div className='flex items-center gap-2'>
              <img 
                ...
                className='w-5 h-5'
              />
              <h2 className='text-xl font-semibold text-gray-800'>About Doctor</h2>
            </div>
          ...
          <div className='bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200'>
            <div className='flex items-center justify-between'>
              ...
              <span className='text-2xl font-bold text-green-600'>${docInfo.fees}</span>
            </div>
          </div>
          ...
          <div className='pt-4 space-y-3'>
            {!currentDoctorAppointment ? (
              <button 
                onClick={() => {
                  ...
                  if (selectedDay && selectedTimeSlot) {
                    ...
                    if (hasAppointmentConflict(docid, selectedDay.fullDate, selectedTimeSlot)) {
                      showWarningToast('You already have an appointment with this doctor at this time');
                      return;
                    }
              ...
              <div className='space-y-4'>
                ...
                <div className='bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200'>
                  <h3 className='text-lg font-semibold text-green-800 mb-2'>Current Appointment</h3>
                  <div className='text-green-700 space-y-1'>
                    <p>📅 <span className='font-medium'>{new Date(currentDoctorAppointment.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span></p>
                    <p>⏰ <span className='font-medium'>{currentDoctorAppointment.time}</span></p>
                    <p>💰 <span className='font-medium'>${currentDoctorAppointment.fee}</span></p>
                    <p>📍 <span className='font-medium'>{currentDoctorAppointment.address?.line1}, {currentDoctorAppointment.address?.line2}</span></p>
                    <p>🆔 <span className='font-medium text-xs'>ID: {currentDoctorAppointment.id}</span></p>
                  </div>
                </div>
                ...
                <button 
                  onClick={() => {
                    const success = cancelAppointment(currentDoctorAppointment.id);
                    if (success) {
                      setCurrentDoctorAppointment(null);
                      showWarningToast(`❌ Appointment cancelled successfully!\n\n👨‍⚕️ Doctor: ${docInfo.name}\n📅 Date: ${new Date(currentDoctorAppointment.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}\n⏰ Time: ${currentDoctorAppointment.time}\n\nYou can book a new appointment anytime.`);
                    }
                  }}
            ...
            {userAppointments.length > 1 && (
              <div className='mt-6 p-4 bg-blue-50 rounded-xl border border-blue-200'>
                <h3 className='text-lg font-semibold text-blue-800 mb-3'>Your Other Appointments ({userAppointments.length - (currentDoctorAppointment ? 1 : 0)})</h3>
                <div className='space-y-2 max-h-32 overflow-y-auto'>
                  {userAppointments
                    .filter(apt => apt.id !== currentDoctorAppointment?.id)
                    .slice(0, 3)
                    .map((apt, index) => (
                    <div key={index} className='flex justify-between items-center p-2 bg-white rounded-lg text-sm'>
                      <div>
                        <p className='font-medium text-blue-800'>Dr. {apt.doctorName}</p>
      ...
      {!currentDoctorAppointment && (
        <div className='mt-8 bg-white rounded-2xl shadow-xl p-6 lg:p-8'>
          ...
          <div className='space-y-4 mb-8'>
            ...
            {selectedDay && (
              <p className='text-green-600 font-medium mt-2'>
                ✓ Selected Date: {selectedDay.dayName}, {selectedDay.dateString}
              </p>
            )}
          </div>
          ...
          {selectedDay && (
            <div className='space-y-4'>
              <h3 className='text-lg font-semibold text-gray-700'>Available Time Slots for {selectedDay.dayName}</h3>
              <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3'>
                {timeSlots.map((time, index) => {
                  const isConflicted = hasAppointmentConflict(docid, selectedDay.fullDate, time);
                  return (
                    <button 
                      key={index}
                      onClick={() => {
                        if (!isConflicted) {
                          ...
                          showWarningToast('This time slot conflicts with your existing appointment');
                        }
                      }}
                    ...
                    >
                      {time}
                      {isConflicted && <div className='text-xs mt-1'>Booked</div>}
                    </button>
                  );
                })}
              </div>
          ...
          <div className='mt-8 p-4 bg-gray-50 rounded-xl'>
            ...
            <div className='space-y-2 text-gray-600'>
              <p><span className='font-medium'>Address:</span> {docInfo.address?.line1}, {docInfo.address?.line2}</p>
              <p><span className='font-medium'>Availability:</span> {docInfo.available ? 'Available' : 'Not Available'}</p>
              <p><span className='font-medium'>Timezone:</span> {Intl.DateTimeFormat().resolvedOptions().timeZone} ({new Date().toLocaleTimeString('en-US', {timeZoneName: 'short'}).split(' ')[2]})</p>
              <p><span className='font-medium'>Working Days:</span> Monday - Saturday</p>
              <p><span className='font-medium'>Working Hours:</span> 9:00 AM - 6:00 PM</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div className='flex items-start'>
              ...
              <div className='ml-3 flex-1'>
                <h3 className='text-sm font-semibold text-gray-900 mb-1'>
                  {toastType === 'success' ? 'Booking Confirmed!' : 'Appointment Cancelled!'}
                </h3>
